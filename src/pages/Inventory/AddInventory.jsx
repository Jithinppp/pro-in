...
import { fetchCategories, fetchModelsByCategory, createAsset, getAssetSequence } from "../../lib/supabase";
...
function AddInventory() {
    ...
    const selectedModelId = watch("models_id");
    ...
    const handleCategoryChange = async (categoryId) => {
        ...
        if (categoryId) {
            ...
            const selectedCategory = categories.find(c => c.id === categoryIdNum || c.id === categoryId);
            if (selectedCategory) {
    ...
    const handleModelChange = async (modelId) => {
        ...
        if (modelId && selectedCategoryId) {
            ...
            const selectedCategory = categories.find(c => c.id === categoryIdNum || c.id === selectedCategoryId);
            const selectedModel = models.find(m => m.id === modelIdNum || m.id === modelId);

            if (selectedCategory && selectedModel) {
                const seqResult = await getAssetSequence(selectedCategory.code, selectedModel.brand_code);
                const nextSequence = seqResult.sequence.toString().padStart(3, "000").slice(-3);
        ...
        <div className="p-4 md:p-6">
            <div className="bg-white rounded-lg border border-gray-200 max-w-full">
                <div className="p-4 border-b border-gray-200">
                    <h1 className="text-xl font-semibold text-gray-900">Add New Inventory Item</h1>
                    ...
                    {previewCode && (
                        <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                            <p className="text-sm text-blue-700">
                                <span className="font-medium">Next Asset Code:</span> {previewCode}
                            </p>
                        </div>
                    )}
                </div>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="p-4 space-y-4">
                    ...
                    <div>
                        <div className="flex items-center justify-between">
                            ...
                            <Link to="/inv/add-category" className="text-xs text-blue-600 hover:text-blue-800">
                                + Add New Category
                            </Link>
                        </div>
                        <select
                    ...
                    <div>
                        <div className="flex items-center justify-between">
                            ...
                            <Link to="/inv/add-model" className="text-xs text-blue-600 hover:text-blue-800">
                                + Add New Model
                            </Link>
                        </div>
                        <select
                        ...
                        {errors.models_id && (
                            <p className="text-red-500 text-xs mt-1">{errors.models_id.message}</p>
                        )}
                    </div>
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <input
                                    type="text"
                                    readOnly
                                    value={purchaseDate ? format(purchaseDate, "MMM dd, yyyy") : ""}
                                    onClick={() => setShowPurchaseDatePicker(!showPurchaseDatePicker)}
                        ...
                        <div>
                            ...
                            <input
                                ...
                                {...register("purchase_price", {
                                    validate: (value) => !value || value >= 0 || "Price must be positive",
                                })}
                                className="w-full px-3 py-2.5 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
                                placeholder="Enter purchase price"
                            />
                            {errors.purchase_price && (
                                <p className="text-red-500 text-xs mt-1">{errors.purchase_price.message}</p>
                            )}
                        </div>
                    </div>
                    ...
                    {(isSubmitting || submitError) && (
                        <div className={`p-3 rounded-lg border ${submitError ? 'bg-red-50 border-red-200' : 'bg-blue-50 border-blue-200'}`}>
                            <p className={`text-sm text-center ${submitError ? 'text-red-700' : 'text-blue-700'}`}>
