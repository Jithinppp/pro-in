...
import { fetchAssets, fetchDeletedAssets, deleteAsset, restoreAsset } from "../../lib/supabase";
...
function InvEquipments() {
  ...
  const [showDeleted, setShowDeleted] = useState(false); // Toggle for deleted items
  const limit = 10;

  const loadAssets = async (query = "", includeDeleted = false) => {
    setLoading(true);
    const result = includeDeleted
      ? await fetchDeletedAssets(page, limit, query)
      : await fetchAssets(page, limit, query, includeDeleted);
    if (result.success) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const result = showDeleted
        ? await fetchDeletedAssets(page, limit, "")
        : await fetchAssets(page, limit, "", showDeleted);
      if (result.success) {
        setAssets(result.assets);
        setTotal(result.total);
      }
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-xl font-semibold text-gray-900">
            {showDeleted ? "Deleted Equipment" : "All Equipment"}
          </h1>
        ...
        <div className="flex gap-2">
          {/* Toggle for deleted items */}
          <button
            onClick={() => {
              setShowDeleted(!showDeleted);
              setPage(1);
            }}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-colors text-sm ${showDeleted
